...
public class DatasUtil {
	...
	public static List<CircleItem> covert2CircleDatas(MyCircleItem myCircleItem) {
		List<CircleItem> circleDatas = new ArrayList<CircleItem>();
		for(MyCircleItem.DataBean bean : myCircleItem.getData()){
			CircleItem item = new CircleItem();
			User user = new User(bean.getUser_id(),bean.getUser_name(),bean.getUser_avatar());
			item.setId(bean.getVideo_id());
			item.setUser(user);
			item.setContent(bean.getVideo_desc());
			item.setCreateTime(bean.getCreatetime());
			item.setIs_praise(bean.getIs_praise());

			List<FavortItem> favortItems = new ArrayList<FavortItem>();
			for(MyCircleItem.DataBean.VideoPraiseListBean praiseListBean : bean.getVideo_praise_list()){
				FavortItem favortItem = new FavortItem();
				User favortItemUser = new User("",praiseListBean.getUser_name(),praiseListBean.getUser_avatar());
				favortItem.setUser(favortItemUser);
				favortItems.add(favortItem);
			}
			item.setFavorters(favortItems);
			List<CommentItem> commentItems = new ArrayList<CommentItem>();
			for(MyCircleItem.DataBean.VideoDiscussListBean discussListBean : bean.getVideo_discuss_list()){
				CommentItem commentItem = new CommentItem();
				commentItem.setContent(discussListBean.getVideo_discuss());
				User commentItemUser = new User("",discussListBean.getUser_name(),discussListBean.getUser_avatar());
				commentItem.setUser(commentItemUser);
				commentItems.add(commentItem);
			}
			item.setComments(commentItems);
			item.setType("2");// 图片

			item.setPhotos(bean.getVideo_img_url());
	...
	public static List<CircleItem> covert2CircleDatas(SomeBodyCircleBean myCircleItem) {
		List<CircleItem> circleDatas = new ArrayList<CircleItem>();
		for(SomeBodyCircleBean.DataBean.ListBean bean : myCircleItem.getData().getList()){
			CircleItem item = new CircleItem();
			User user = new User(bean.getUser_id(),bean.getUser_name(),bean.getUser_avatar());
			item.setId(bean.getVideo_id());
			item.setUser(user);
			...
			List<FavortItem> favortItems = new ArrayList<FavortItem>();
			for(SomeBodyCircleBean.DataBean.ListBean.VideoPraiseListBean praiseListBean : bean.getVideo_praise_list()){
				FavortItem favortItem = new FavortItem();
				User favortItemUser = new User("",praiseListBean.getUser_name(),praiseListBean.getUser_avatar());
				favortItem.setUser(favortItemUser);
				favortItems.add(favortItem);
			...
			item.setFavorters(favortItems);
			List<CommentItem> commentItems = new ArrayList<CommentItem>();
			for(SomeBodyCircleBean.DataBean.ListBean.VideoDiscussListBean discussListBean : bean.getVideo_discuss_list()){
				CommentItem commentItem = new CommentItem();
				commentItem.setContent(discussListBean.getVideo_discuss());
				User commentItemUser = new User("",discussListBean.getUser_name(),discussListBean.getUser_avatar());
				commentItem.setUser(commentItemUser);
	...
	public static FavortItem createCurUserFavortItem() {
		FavortItem item = new FavortItem();
		item.setId(String.valueOf(favortId++));
		User curUser = new User( TCUserMgr.getInstance().getUserId(), TCUserMgr.getInstance().getNickname(), TCUserMgr.getInstance().getHeadPic());
