...
import com.leichui.shortviedeo.activity.InfoDetailsActivity;
import com.leichui.shortviedeo.activity.PublishActivity;
...
public class CircleAdapter extends BaseRecycleViewAdapter {
    ...
    public CircleAdapter(Context context,int headSize){
        this.context = context;
        HEADVIEW_SIZE = headSize;
    }

    @Override
    public int getItemViewType(int position) {
        if(position == 0 && HEADVIEW_SIZE == 1){
            return TYPE_HEAD;
        }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, final int position) {
        ...
        if(getItemViewType(position)==TYPE_HEAD){
            ...
            holder.myHead.setImageURI(TCUserMgr.getInstance().getHeadPic());
            holder.myName.setText(TCUserMgr.getInstance().getNickname());
            holder.paizhaoRl.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    UtKt.startMyActivity(context, PublishActivity.class);
                }
            });
        }else{
...
//            Glide.with(context).load(headImg).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.color.bg_no_photo).transform(new GlideCircleTransform(context)).into(holder.headIv);
            holder.headIv.setImageURI(headImg);
            holder.headIv.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    UtKt.startMyActivityWithOneParm(context, InfoDetailsActivity.class,"userId",circleItem.getUser().getId());
                }
            });
...
//                            Toast.makeText(TCApplication.instance, userName + " &id = " + userId, Toast.LENGTH_SHORT).show();
                        }
                    });
...
//                                    config.circleId = ((CircleItem) datas.get(circlePosition)).getId();
            ...
            //判断是否已点赞
            String curUserFavortId = circleItem.getIs_praise();
            if("1".equals(curUserFavortId)){
...
//                    });
    ...
    public class HeaderViewHolder extends RecyclerView.ViewHolder{
        ...
        public SimpleDraweeView myHead;
        public TextView myName;
        public RelativeLayout paizhaoRl;
        public HeaderViewHolder(View itemView) {
            super(itemView);
            myHead = (SimpleDraweeView) itemView.findViewById(R.id.myHead);
            myName = (TextView) itemView.findViewById(R.id.myName);
            paizhaoRl = (RelativeLayout) itemView.findViewById(R.id.paizhaoRl);
        }
    }
...
//                    }
            ...
            headIv = (SimpleDraweeView) itemView.findViewById(R.id.headIv);
