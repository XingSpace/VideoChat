...
public class TCUserMgr {
    ...
    private String currentVideoPic;
    ...
    public String getCurrentVideoPic() {
        return currentVideoPic;
    }

    public void setCurrentVideoPic(String currentVideoPic) {
        this.currentVideoPic = currentVideoPic;
    }
...
//                message = jsonObject.getString("msg");
    ...
    public boolean hasUser() {
        ...
        Log.e("asd","------mToken:" + mToken);
        return mToken== null || !TextUtils.isEmpty(mToken);
...
//        uploadUserInfo(callback);
    ...
    public void getVodSig(final Callback callback) {
        try {
            JSONObject body = new JSONObject();
            body.put("token",mToken);
            myRequest("index.php?s=/Home/UpVideo/getSign", body, new HttpCallback("get_vod_sign", callback));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//                    .addHeader("Liteav-Sig", sig)
    ...
    public void myRequest(String cmd, @NonNull JSONObject body, okhttp3.Callback callback) {
        long now = System.currentTimeMillis() / 1000;
        try {
            String strBody = body.toString();

            Request request = new Request.Builder()
                    .url(UGCKitConstants.MYAPP_SVR_URL + cmd)
                    .post(RequestBody.create(MediaType.parse("application/json; charset=utf-8"), strBody))
                    .build();
            mHttpClient.newCall(request).enqueue(callback);
            TXLog.d(TAG, "xsp_process: " + request.toString() + ", Body" + strBody);
...
//        }
    ...
    private void loadUserInfo() {
        ...
        mToken = settings.getString(UGCKitConstants.USER_TOKEN, "");
    }
    ...
    private void saveUserInfo() {
        ...
        editor.putString(UGCKitConstants.USER_TOKEN, mToken);
        editor.commit();
    }
    ...
    private void clearUserInfo() {
        ...
        editor.putString(UGCKitConstants.USER_TOKEN, "");
        editor.commit();
    }
}
